...
void loop() {
  ...
  digitalWrite(secondScreenCS, LOW);
  uint16_t c2, *destPtr2;
  int16_t  bx2  = minx - (int)ballx, // X relative to ball bitmap (can be negative)
  ...
  for(y=0; y<height; y++) { // For each row...
    ...
    for(x=0; x<width; x++) {
      if((bx12 >= 0) && (bx12 < BALLWIDTH) &&  // Is current pixel row/column
        ...
        // Yes, do ball compositing math...
        p2 = ball[by2][bx12 / 2];                // Get packed value (2 pixels)
        c2 = (bx12 & 1) ? (p2 & 0xF) : (p2 >> 4); // Unpack high or low nibble
        if(c2 == 0) { // Outside ball - just draw grid
          c2 = background[bgy2][bgx12 / 8] & (0x80 >> (bgx12 & 7)) ? GRIDCOLOR : BGCOLOR;
        } else if(c2 > 1) { // In ball area...
          c2 = palette[c2];
        } else { // In shadow area...
          c2 = background[bgy2][bgx12 / 8] & (0x80 >> (bgx12 & 7)) ? GRIDSHADOW : BGSHADOW;
        }
      } else { // Outside ball bitmap, just draw background bitmap...
        c = background[bgy2][bgx12 / 8] & (0x80 >> (bgx12 & 7)) ? GRIDCOLOR : BGCOLOR;
      }
      *destPtr2++ = c2<<8 | c2>>8; // Store pixel colour
      bx12++;  // Increment bitmap position counters (X axis)
      bgx12++;
    }
    tft.pushPixelsDMA(&renderbuf[bufIdx2][0], width); // Push line to screen
    // Push line to screen (swap bytes false for STM/ESP32)
    //tft.pushPixels(&renderbuf[bufIdx][0], width);

    bufIdx2 = 1 - bufIdx2;
    by2++; // Increment bitmap position counters (Y axis)
    bgy2++;
  }
  //if (random(100) == 1) delay(2000);
  tft.endWrite();
